...
df_2011 = df_2011[['Unnamed: 0.1', 'clusters', 'clusters_2011_edited']]
df_2011['idx'] = df_2011['Unnamed: 0.1'].apply(lambda x : int(x[:-2]))
df_2018 = df_2018[['Unnamed: 0', 'clusters', 'clusters']]
df_2018['idx'] = df_2018['Unnamed: 0'].apply(lambda x : int(x[:-2]))
df_2021 = df_2021[['Unnamed: 0.1', 'clusters', 'clusters_2021_edited']]
...
df_2011_class = pd.get_dummies(df_2011_[['clusters_2011_edited']].astype('category'))
...
df_2018_class = pd.get_dummies(df_2018_[['clusters']].astype('category'))
...
# one hot encode classes 2018
df_2021_class = pd.get_dummies(df_2021_[['clusters_2021_edited']].astype('category'))
...
for feature in features:
    try:
        new_column = 'proportion_%s' % str(feature)
        t0_column = 'clusters_2011_edited_%s' % str(feature)
        t1_column = 'clusters_2021_edited_%s' % str(feature)
        merge_all[new_column] = merge_all[t1_column] - merge_all[t0_column]
    except:
        merge_all[new_column] = [np.nan for i in range(4830)]
...
ax.set_xticklabels([i.split('_')[-1] for i in df.columns])
ax.set_yticks(np.arange(18))
ax.set_yticklabels([i.split('_')[-1] for i in df.columns])
ax.tick_params(axis="x", bottom=True, top=False, labelbottom=True, labeltop=False)
plt.setp([tick.label1 for tick in ax.xaxis.get_major_ticks()], rotation=90,
          ha="right", va="center", rotation_mode="anchor")
f.tight_layout()
plt.show()
plt.savefig('outputs/corr_matrix_lsoa.png',bbox_inches="tight", dpi=150)
